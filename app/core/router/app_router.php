    ...
    if (isset($_GET['url'])) {
        ...
        if (is_dir(APP_PATH . '/public/views/' . $url_folder_name)) {
            if (file_exists(APP_PATH . '/public/views/' . $url_folder_name . '/index.php')) {
                ...
                exit("Terjadi kesalahan, file <strong>index.php</strong> pada folder <strong>" . $url_folder_name . "</strong> tidak ditemukan.");
            }
        } else {
            // 404 error
            http_response_code(404);
            require_once("public/views-template/404.php");
            exit;
        }
    } else {
        // Check if the folder exists
